...
describe(
    ...
    () => {it("Reads reflection table", () => {
      ...
      const reflectionSchemaBuffer: Buffer =
          readFileSync(`${__dirname}/reflection.bfbs`);
      ...
      expect(schemaObject['fbs_files'][0]['filename'].substr(-14))
          .toEqual('reflection.fbs');
      expect(schemaObject['fbs_files'][0]['included_filenames'].length)
          .toEqual(0);
      ...
      // * Default scalars
      {
        ...
        const docOffsets =
            [ builder.createString('abc'), builder.createString('def') ];
        const docVector =
            EnumVal.createDocumentationVector(builder, docOffsets);
